...
func (t *Tree) pathToRoot(child *BTNode) int {
	length := 0
	c := child
	for c != t.Root {
		c = c.Parent
...
func (t *Tree) findRange(key_start, key_end int, verbose bool, returned_keys []int, returned_pointers []interface{}) int {
	...
	for n != nil {
		...
		n, _ = n.Pointers[order-1].(*BTNode)
		i = 0
	}
	return num_found
}

func (t *Tree) findLeaf(key int, verbose bool) *BTNode {
	i := 0
	c := t.Root
	if c == nil {
		if verbose {
...
func makeNode() (*BTNode, error) {
	new_node := new(BTNode)
	if new_node == nil {
		return nil, errors.New("Error: BTNode creation.")
	}
...
func getLeftIndex(parent, left *BTNode) int {
	left_index := 0
...
func insertIntoLeaf(leaf *BTNode, key int, pointer *Record) {
...
func (t *Tree) insertIntoLeafAfterSplitting(leaf *BTNode, key int, pointer *Record) error {
	var new_leaf *BTNode
...
func insertIntoNode(n *BTNode, left_index, key int, right *BTNode) {
	var i int
...
func (t *Tree) insertIntoNodeAfterSplitting(old_node *BTNode, left_index, key int, right *BTNode) error {
	var i, j, split, k_prime int
	var new_node, child *BTNode
	var temp_keys []int
	...
	for i = 0; i <= new_node.NumKeys; i++ {
		child, _ = new_node.Pointers[i].(*BTNode)
...
func (t *Tree) insertIntoParent(left *BTNode, key int, right *BTNode) error {
	var left_index int
	parent := left.Parent
...
func (t *Tree) insertIntoNewRoot(left *BTNode, key int, right *BTNode) error {
	t.Root, err = makeNode()
	if err != nil {
		return err
	}
...
func getNeighbourIndex(n *BTNode) int {
	var i int
...
func removeEntryFromNode(n *BTNode, key int, pointer interface{}) *BTNode {
...
func (t *Tree) adjustRoot() {
	...
	if !t.Root.IsLeaf {
		new_root, _ = t.Root.Pointers[0].(*BTNode)
		new_root.Parent = nil
	} else {
		new_root = nil
	}
...
func (t *Tree) coalesceNodes(n, neighbour *BTNode, neighbour_index, k_prime int) {
	...
	if !n.IsLeaf {
		...
		for i = 0; i < neighbour.NumKeys+1; i++ {
			tmp, _ = neighbour.Pointers[i].(*BTNode)
...
func (t *Tree) redistributeNodes(n, neighbour *BTNode, neighbour_index, k_prime_index, k_prime int) {
	var i int
	var tmp *BTNode
	...
	if neighbour_index != -1 {
		...
		if n.IsLeaf {
			...
			tmp, _ = n.Pointers[n.NumKeys+1].(*BTNode)
			tmp.Parent = n
...
func (t *Tree) deleteEntry(n *BTNode, key int, pointer interface{}) {
	var min_keys, neighbour_index, k_prime_index, k_prime, capacity int
	var neighbour *BTNode
	...
	if neighbour_index == -1 {
		neighbour, _ = n.Parent.Pointers[1].(*BTNode)
	} else {
		neighbour, _ = n.Parent.Pointers[neighbour_index].(*BTNode)
	}
